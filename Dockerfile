...
RUN mkdir -p /tmp/docker && mkdir -p /app
RUN cd /tmp/docker && wget -O caddy.tar.gz "https://caddyserver.com/download/build?os=linux&arch=amd64&features=cors%2Cexpires%2Cfilter%2Cipfilter%2Cminify%2Cratelimit%2Crealip" && \
    tar xfz caddy.tar.gz && mv caddy /usr/local/bin

# Install docker-gen
RUN cd /tmp/docker && wget -O docker-gen.tar.gz "https://github.com/jwilder/docker-gen/releases/download/${DOCKER_GEN_VERSION}/docker-gen-alpine-linux-amd64-${DOCKER_GEN_VERSION}.tar.gz" && \
    tar xfz docker-gen.tar.gz && mv docker-gen /usr/local/bin
